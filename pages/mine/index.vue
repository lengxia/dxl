				...
				<view class="user-card">
					<view class="avatar-wrapper" @click="handleAvatarClick">
						<view class="avatar-ring">
							<image :src="userInfo.avatar || '/static/logo.png'" class="avatar-image"
								mode="aspectFill" />
						</view>
					...
					<view class="user-info">
						...
						<view class="user-motto" v-if="!isAnonymous">潜心修行，福慧双增</view>
					</view>
				...
				<view class="profile-form">
					...
					<view class="avatar-choose-box">
						<button class="avatar-btn" open-type="chooseAvatar" @chooseavatar="onChooseAvatar">
							<image :src="tempAvatar || userInfo.avatar || '/static/logo.png'" class="temp-avatar"
								mode="aspectFill" />
	...
	export default {
		...
		componentShow() {
			...
			async uploadAvatar(filePath) {
				...
				try {
					const fileID = await this.uploadToCloud(filePath);

					// 使用 auth.js 统一接口更新头像
					// 更新成功后，auth.js 会更新 Vuex，Vuex 会触发 watch，watch 会触发 loadUserInfo 更新视图
			...
			async handleWechatAuth(isSilent = false) {
				...
				if (success) {
					...
					if (result.type === 'register' || isDefaultNickname || isNoAvatar) {
						// 初始化临时变量，避免用户只需修改一项时被校验拦截
						this.tempNickname = isDefaultNickname ? '' : userInfo.nickname;
						// 如果有头像，预填原头像
						this.tempAvatar = userInfo.avatar || '';
