		...
		<view class="page-wrapper">
			...
			<view class="page-content" :style="{paddingTop: vuex_custom_bar_height + 'px'}" v-if="!isLogin">
				<view class="login-guide-card">
					...
					<tn-button shape="round" backgroundColor="#C9A86C" fontColor="#FFFFFF" width="280rpx" shadow
						@click="goBack">
						去登录
					</tn-button>
				</view>
			...
			<view class="page-content" :style="{paddingTop: vuex_custom_bar_height + 'px'}" v-else>
				...
				<view class="list-section">
					...
					<view v-if="listData.length > 0" class="diary-list">
						<view v-for="(item, index) in listData" :key="index" class="diary-card"
						...
						<view v-else-if="!pagination.hasMore && listData.length > 0" class="load-more">
							<text>没有更多了</text>
						</view>
					</view>
...
<script>
	...
	export default {
		...
		computed: {
			...
			totalMerit() {
				return this.listData.reduce((sum, d) => sum + (d.merit_points || 0), 0);
			},
			totalCount() {
		...
		methods: {
			...
			async fetchListData(params) {
				...
				const res = await this.$api.call('getDiaries', {
					uid: this.currentUid,
					page: params.page,
