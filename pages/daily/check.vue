		...
		<view class="page-content" :style="{paddingTop: vuex_custom_bar_height + 'px'}">
			...
			<tn-form :model="form" ref="checkForm">
				...
				<view class="form-section">
					...
					<view class="section-card">
						<view class="checkbox-grid">
							<view class="checkbox-item" :class="{active: form.mind_check.anxiety}"
							...
							<view class="checkbox-item" :class="{active: form.mind_check.greed}"
							...
							<view class="checkbox-item" :class="{active: form.mind_check.arrogance}"
							...
							<view class="checkbox-item" :class="{active: form.mind_check.anger}"
						...
						<view class="form-row">
							...
							<view class="row-control">
								<view class="radio-group">
									<view class="radio-item" :class="{active: form.body_check.stomach_status === '舒适'}"
										@click="form.body_check.stomach_status = '舒适'">舒适</view>
									<view class="radio-item negative"
										:class="{active: form.body_check.stomach_status === '不适'}"
										@click="form.body_check.stomach_status = '不适'">不适(-5)</view>
								</view>
							</view>
						</view>
					</view>
				...
				<view class="form-section">
					...
					<view class="section-card">
						...
						<view class="form-row">
							...
							<view class="row-control">
								<input v-model="form.practice_check.writing_words" type="number" placeholder="字数"
									class="text-input" />
				...
				<view class="form-section">
					...
					<view class="section-card">
						<textarea v-model="form.notes" placeholder="今日修行有何收获与感悟..." class="notes-textarea"
			...
			<view class="submit-section">
				<tn-button backgroundColor="#3D8B8F" fontColor="#FFFFFF" shape="round" width="100%" shadow fontBold
					@click="submit">
					完成今日功课
				</tn-button>
			</view>
...
<script>
	import pageMixin from '@/libs/form-page-mixin.js'
	import {
		getTodayStr
	...
	export default {
		...
		methods: {
			...
			async loadExistingData(date) {
				const res = await this.$api.call('getDailyTask', {
					date,
					uid: this.currentUid
				}, {
					showLoading: true,
			...
			async submit() {
				...
				const res = await this.$api.call('submitDailyTask', data, {
					showLoading: true,
					loadingText: '记录中'
				})
