		...
		<view class="page-wrapper">
			...
			<view class="page-content" :style="{paddingTop: vuex_custom_bar_height + 'px'}" v-if="!isLogin">
				<view class="login-guide-card">
					...
					<tn-button shape="round" backgroundColor="#3D8B8F" fontColor="#FFFFFF" width="280rpx" shadow
						@click="goBack">
						去登录
					</tn-button>
				</view>
			...
			<view class="page-content" :style="{paddingTop: vuex_custom_bar_height + 'px'}" v-else>
				...
				<view class="calendar-section">
                  ...
                  'is-clickable': !day.isOtherMonth && day.day <= new Date().getDate()
				...
				<view class="today-section">
					<view class="section-header">
						...
						<tn-tag :backgroundColor="todayTask ? '#E8F5E9' : '#FFF3E0'"
							:fontColor="todayTask ? '#2E7D32' : '#E65100'" shape="circle" size="sm">
							{{ todayTask ? '已完成' : '未完成' }}
						</tn-tag>
					</view>
...
<script>
	...
	export default {
		...
		// 使用 onPageShow 替代 onShow，让 mixin 的 onShow 能够正常工作
		// mixin 会自动处理登录检查和节流，然后调用这个钩子
		onPageShow() {
			// 每次页面显示时更新今天的日期
		...
		methods: {
			...
			loadStats() {
				// 从 Vuex 加载用户统计数据
				const user = this.$store.state.vuex_user;
				if (user && user.dao_profile) {
					this.stats = {
						continuous_days: user.dao_profile.continuous_days || 0,
						total_score: user.dao_profile.total_merit || 0,
						level: user.dao_profile.level || 1
					};
				}
			},
			goCheck() {
			...
			onDayClick(day) {
				...
				if (day.isOtherMonth || day.day > today) {
					return;
				}

				// 构造日期字符串
				const dateStr =
					`${this.currentYear}-${String(this.currentMonth).padStart(2,'0')}-${String(day.day).padStart(2,'0')}`;
			...
			async loadCalendarData() {
				...
				const res = await this.$api.call('getMonthlyCheckIns', {
						uid: this.currentUid,
						yearMonth
					}, {
						showLoading: false
