<template>
	<view class="start-index">
		...
		<tn-tabbar v-model="currentIndex" :list="tabbarList" activeColor="#3D8B8F" inactiveColor="#B2BEC3"
			activeIconColor="#3D8B8F" :animation="true" :safeAreaInsetBottom="true" @change="switchTabbar"></tn-tabbar>

		<!-- 首次启动欢迎弹窗 -->
		<tn-modal v-model="showWelcomeModal" :custom="true" :showCloseBtn="false" padding="0" width="600rpx"
			radius="24">
			<view class="welcome-modal">
				...
				<view class="welcome-content">
					...
					<tn-button shape="round" backgroundColor="#3D8B8F" fontColor="#FFFFFF" width="100%" height="90rpx"
						shadow @click="confirmWelcome">
						立即开启
					</tn-button>
				</view>
			</view>
...
<script>
	...
	onLoad(options) {
		...
		onShow() {
			// 页面显示时，触发当前tab组件的onShow
			this._triggerComponentShow(this.currentIndex)
		},
		methods: {
			// 切换导航
			switchTabbar(index) {
			...
			// 触发组件onShow
			_triggerComponentShow(index) {
				this.$nextTick(() => {
					const refName = index === 0 ? 'home' : 'mine'
					const component = this.$refs[refName]
					if (component && typeof component.componentShow === 'function') {
