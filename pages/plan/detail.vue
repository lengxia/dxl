	...
	<view class="detail-page">
		...
		<view class="page-content" :style="{paddingTop: (vuex_custom_bar_height + 20) + 'px'}" v-if="plan">
			...
			<view class="goals-section">
				...
				<view v-for="(goal, index) in plan.goals" :key="index" class="goal-card">
					...
					<view class="goal-progress-bar">
						<view class="progress-fill" :class="[categoryClass(goal.category)]"
							:style="{width: calculateProgress(goal) + '%'}"></view>
					</view>

					<view class="goal-actions">
						<view :class="['check-btn', goal.completed_days >= goal.target_days ? 'completed' : '']"
	...
	export default {
		...
		methods: {
			...
			async incrementProgress(index) {
				...
				const res = await this.$api.call('updateMonthlyPlan', {
					id: this.id,
				...
				uni.showModal({
					...
					success: async (res) => {
						if (res.confirm) {
							...
							const res = await this.$api.call('deleteMonthlyPlan', {
								id: this.id
							}, {
								showLoading: false
							})
