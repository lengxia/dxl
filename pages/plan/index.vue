<template>
	<view class="plan-page tn-safe-area-inset-bottom">
			...
			<view class="page-content" :style="{paddingTop: vuex_custom_bar_height + 'px'}">
				...
				<view class="list-section">
					...
					<view v-if="plans.length > 0" class="plan-list">
						<view v-for="(item, index) in plans" :key="index" class="plan-card" @click="goDetail(item._id)">
							...
							<view class="plan-progress">
								<view class="progress-bar">
									<view class="progress-fill" :style="{width: calculateProgress(item.goals) + '%'}">
									</view>
								</view>
		...
		<!-- 时间选择器（年月） -->
		<tn-picker v-model="showCalendar" mode="time" :params="{year: true, month: true, day: false}"
			@confirm="onDateChange"></tn-picker>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			async fetchListData(params) {
				const res = await this.$api.call('getMonthlyPlans', {
					uid: this.currentUid,
			...
			calculateTotalProgress() {
				...
				this.progress = Math.round(totalPercent / this.listData.length);
			}
		}
	}
</script>
