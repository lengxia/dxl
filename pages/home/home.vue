				...
				<view class="greeting-content">
					<view class="greeting-row">
						...
						<view class="avatar-box" @click="tn('/pages/mine/mine')">
							<image :src="userAvatar || '/static/logo.png'" class="avatar-img" mode="aspectFill" />
						</view>
					</view>
			...
			<view class="stats-section">
				<view class="stats-card">
					...
					<view class="stat-item">
						<view class="stat-value warm">{{ stats.level_name }}</view>
	...
	export default {
		...
		methods: {
			...
			componentShow() {
				this.setGreeting();
				// 重新检查状态
				this.checkDailyCheckIn(this.LastDailyDate);
			},
			tn(url) {
				uni.navigateTo({
					url
			...
			loadUserInfo(user) {
				...
				// 处理修行数据
				if (user.dao_profile) {
					this.stats.days = user.dao_profile.continuous_days || 0;
